    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 mb-3">
          ...
          <span className="flex items-center gap-1 text-[11px] text-ink-muted">
            <DollarSign size={10} /> {fmt(trip.budget)}
          </span>
          <span className="flex items-center gap-1 text-[11px] text-ink-muted">
        ...
        <p className="text-[10px] text-ink-faint mb-3">
          Saved {new Date(trip.savedAt || trip.createdAt).toLocaleDateString('en-IN', { day:'numeric', month:'short', year:'numeric' })}
        </p>
...
export default function SavedTripsPage() {
  ...
  const { trips, loading, fetchTrips, deleteTrip, loadTrip } = useSavedTripsStore()
  ...
  const handleView = async (trip) => {
    // Try to load full trip from API (with itineraryJson)
    if (trip.itineraryJson) {
      try {
        const parsed = JSON.parse(trip.itineraryJson)
        setItinerary(parsed)
        navigate('/itinerary')
        return
      } catch { /* fall through */ }
    ...
    const fullTrip = await loadTrip(trip.id)
    if (fullTrip?.itineraryJson) {
      try {
        const parsed = JSON.parse(fullTrip.itineraryJson)
        setItinerary(parsed)
        navigate('/itinerary')
        return
      } catch { /* fall through */ }
    ...
    // Fallback — navigate to planner
    showToast({ message: 'No itinerary data found — redirecting to planner', type: 'info' })
    navigate('/plan/setup')
  }
      ...
      <div className="max-w-5xl mx-auto w-full px-4 py-8 pb-24 md:pb-8">
        ...
        {loading && trips.length === 0 ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1, 2, 3].map((i) => (
              <SkeletonCard key={i} height="h-64" lines={3} />
            ))}
          </div>
        ) : trips.length === 0 ? (
          <EmptyState
